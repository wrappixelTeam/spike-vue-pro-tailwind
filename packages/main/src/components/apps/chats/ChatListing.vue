    ...
    <perfect-scrollbar class="lgScroll h-100">
        <v-list>
            ...
            >
                ...
                <!---Name-->
                <v-list-item-title class="text-sm textPrimary w-100 font-semibold">{{ chat.name }}</v-list-item-title>
                <!---Subtitle-->
                <v-sheet v-if="chat.chatHistory.slice(-1)[0].type == 'img'">
                    <small class="textPrimary text-xs">Sent a Photo</small>
                </v-sheet>
                <div class="text-xs textPrimary mt-1 text-truncate w-100" v-else>
                    {{ chat.chatHistory.slice(-1)[0].msg }}
                </div>
                <!---Last seen--->
                <template v-slot:append>
                    <div class="flex flex-column text-right w-25">
                        <small class="textPrimary text-xs">
                            {{
