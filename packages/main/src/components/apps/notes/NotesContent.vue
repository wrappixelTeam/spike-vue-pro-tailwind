...
<template>
    ...
    <v-sheet>
        <v-sheet class="py-3 pl-6 pr-4 flex items-center">
            <h4 class="text-base font-semibold">Edit Notes</h4>
            <div class="ml-auto"><AddNote /></div>
        </v-sheet>
        <v-divider></v-divider>
        <v-sheet v-if="getNote">
            <v-sheet class="pa-6">
                <h4 class="text-base mb-4">Change Title</h4>
                <v-textarea outlined name="Note" v-model="getNote.title"></v-textarea>

                <h4 class="text-base mt-4 mb-4">Change Notes Color</h4>
