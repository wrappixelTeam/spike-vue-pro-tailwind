    ...
    <v-list two-lines>
        <template v-for="(custom, i) in customs" :key="i">
            <v-list-item active-color="primary" class="py-3">
                <v-list-item-title class="text-sm">{{ custom.title }}</v-list-item-title>
                <v-list-item-subtitle v-if="custom.profit" class="text-xs text-success text-high-emphasis"
                    >{{ custom.percent }}% Profit</v-list-item-subtitle
                >
                <v-list-item-subtitle v-else class="text-xs text-error "
                    >{{ custom.percent }}% Loss</v-list-item-subtitle
                >
                <template v-slot:append>
                    <v-list-item-subtitle class="text-sm text-high-emphasis mr-3"> ${{ custom.price }}</v-list-item-subtitle>
